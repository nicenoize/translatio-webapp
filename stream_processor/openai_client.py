...
class OpenAIClient:
    def __init__(self, api_key: str, translated_audio_pipe: str):
        ...
        # Initialize playback buffer and queues
        ...
        # Initialize a lock for reconnecting to prevent race conditions
        self.is_reconnecting = False
        self.reconnect_lock = asyncio.Lock()

        # Initialize task attributes
        self.playback_task = None
        self.ffmpeg_writer_task = None

    def create_named_pipe(self, pipe_name):
        if os.path.exists(pipe_name):
            try:
                st = os.stat(pipe_name)
                if not stat.S_ISFIFO(st.st_mode):
                    self.logger.warning(f"File {pipe_name} exists and is not a named pipe. Removing and recreating.")
                    os.remove(pipe_name)
                    os.mkfifo(pipe_name)
                    self.logger.info(f"Recreated named pipe: {pipe_name}")
                else:
                    self.logger.info(f"Named pipe already exists: {pipe_name}")
            except Exception as e:
                self.logger.error(f"Error checking/removing existing file {pipe_name}: {e}")
                raise
        else:
            try:
                os.mkfifo(pipe_name)
        ...
        try:
            async with aiofiles.open('input_audio_pipe', 'rb') as pipe:
                while True:
                    ...
                    self.logger.debug(f"Enqueued audio chunk of size: {len(base64_audio)} bytes")
        except Exception as e:
                ...
                if event_type == "input_audio_buffer.speech_started":
                    ...
                    if audio_data:
                        try:
                            ...
                            sequence = self.playback_sequence  # Assign current sequence number
                            await self.enqueue_audio(sequence, decoded_audio)
                            self.playback_sequence += 1  # Increment for the next chunk
    ...
    async def audio_playback_handler(self):
        ...
        while True:
            ...
            if sequence is None and audio_data is None:
                self.logger.info("Playback handler received shutdown signal.")
                self.playback_queue.task_done()
                break  # Allows graceful shutdown

            if not isinstance(sequence, int):
                self.logger.error(f"Invalid sequence type: {type(sequence)}. Expected int.")
                self.playback_queue.task_done()
                continue
    ...
    async def process_playback_chunk(self, sequence: int, audio_data: bytes):
        ...
        try:
            ...
            play_obj = sa.play_buffer(audio_samples, 1, 2, 24000)
    ...
    async def ffmpeg_writer(self):
        """Dedicated coroutine to write translated audio data to translated_audio_pipe from ffmpeg_queue"""
        buffer = {}
        while True:
            sequence, audio_data = await self.ffmpeg_queue.get()
            if sequence is None and audio_data is None:
            ...
            if sequence == self.current_sequence:
                try:
                    async with aiofiles.open(self.translated_audio_pipe, 'ab') as pipe:
                        await pipe.write(audio_data)
                ...
                while self.current_sequence in buffer:
                    ...
                    try:
                        async with aiofiles.open(self.translated_audio_pipe, 'ab') as pipe:
                            await pipe.write(buffered_data)
                        self.current_sequence += 1
    ...
    async def disconnect(self):
        ...
        if self.playback_queue:
            await self.playback_queue.put((None, None))
        if self.ffmpeg_queue:
            await self.ffmpeg_queue.put((None, None))
        ...
        if self.ffmpeg_writer_task:
            self.ffmpeg_writer_task.cancel()
            try:
    ...
    async def run_client(self):
        ...
        try:
            ...
            # Start reading from input_audio_pipe
